    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 backdrop-blur-sm transition-colors hover:border-white/20">
      <div className="flex items-start justify-between">
        <div>
          ...
          <NumberTicker value={value || 0} className="mt-2 block text-3xl font-bold text-white" />
        </div>
        <div className={`flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.06] ${accent}`}>
          {icon}
        </div>
      </div>
...
function DashboardTab({ apiBase }) {
  ...
  return (
    <div className="space-y-6">
      {/* Stat cards — the exact counts from /ingest + /api/stats */}
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          icon={<IconChart className="h-5 w-5 text-red-300" />}
          label="Total Tasks"
          value={stats?.total}
          sub={`${stats?.processed || 0} emails processed`}
        />
        <StatCard
          icon={<IconCheck className="h-5 w-5 text-emerald-300" />}
          label="Created"
          value={stats?.created}
          sub="tasks created by routing"
        />
        <StatCard
          icon={<IconClock className="h-5 w-5 text-blue-300" />}
          label="Updated"
          value={stats?.updated}
          sub={`${stats?.threads?.updated_multiple_times || 0} threads updated >1×`}
        />
        <StatCard
          icon={<IconAlert className="h-5 w-5 text-amber-300" />}
          label="Skipped"
          value={stats?.skipped}
          sub="spam · newsletters · auto-replies"
        />
      </div>
      ...
      {stats && (
        <div className="flex flex-wrap gap-3">
          {[
            ...
            { icon: <IconClock className="h-3.5 w-3.5" />, label: 'Low confidence (<60%)', value: stats.confidence?.low_confidence_count },
          ].map((chip, idx) => (
            <div
              key={idx}
              ...
              {filteredTasks.map((task) => (
                <tr key={task.task_id} className="transition-colors hover:bg-white/[0.04]">
                  <td className="px-6 py-3.5">
                    <p className="max-w-[240px] truncate text-sm font-medium text-slate-200">{task.title}</p>
                    <p className="mt-0.5 font-mono text-[10px] text-slate-600">{task.task_id}</p>
                  </td>
                  <td className="px-6 py-3.5">
                    <span className="inline-flex rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-0.5 text-xs text-slate-300">
