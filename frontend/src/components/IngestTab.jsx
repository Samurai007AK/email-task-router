...
function generateSampleEmails() {
  ...
  // Generate 250 emails with realistic distribution
  const distribution = {
    rfp: 40, smb: 35, marketing: 25, alliances: 15, finance: 25,
    ooo: 30, newsletter: 30, spam: 30, triage: 10, hinglish: 10
  }
  ...
  for (const [type, count] of Object.entries(distribution)) {
    for (let i = 0; i < count; i++) {
      ...
      emails.push({
        ...
        body: isReply ? `Correction to our earlier note — the board has approved an increased budget of Rs. ${budget + 10} lakhs.\n\nRegards,\n${name}` : body,
        received_at: `2026-08-${String(day).padStart(2, '0')}T${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00+05:30`,
        attachments: type === 'rfp' ? ['RFP_document.pdf'] : type === 'finance' ? ['invoice.pdf'] : [],
        is_reply: isReply
      })
      id++
    }
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">Paste Email JSON</h2>
        ...
        <textarea
          ...
          className="w-full h-64 p-4 border border-gray-300 rounded-lg font-mono text-sm resize-y focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          placeholder='[{"email_id": "em_001", "thread_id": "th_001", ...}]'
        />
        <div className="mt-4 flex items-center justify-between">
          <span className="text-sm text-gray-500">
            {parsedEmails.length} emails parsed
          </span>
          <button
      ...
      {ingestResult && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Ingestion Results</h2>
          <div className="grid grid-cols-5 gap-4">
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold text-gray-900">{ingestResult.processed}</div>
              <div className="text-sm text-gray-500">Processed</div>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{ingestResult.tasks_created}</div>
              <div className="text-sm text-gray-500">Tasks Created</div>
            </div>
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{ingestResult.tasks_updated}</div>
              <div className="text-sm text-gray-500">Tasks Updated</div>
            </div>
            <div className="text-center p-4 bg-yellow-50 rounded-lg">
              <div className="text-2xl font-bold text-yellow-600">{ingestResult.skipped}</div>
              <div className="text-sm text-gray-500">Skipped</div>
            </div>
            <div className="text-center p-4 bg-red-50 rounded-lg">
              <div className="text-2xl font-bold text-red-600">{ingestResult.errors?.length || 0}</div>
      ...
      {parsedEmails.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            Raw Email Data ({parsedEmails.length} emails)
          </h2>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 sticky top-0">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Thread</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {parsedEmails.slice(0, 50).map((email, idx) => (
                  <tr key={email.email_id || idx} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-gray-900">{email.from_name}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{email.from_email}</td>
                    <td className="px-4 py-3 text-sm text-gray-900 max-w-xs truncate">{email.subject}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{email.received_at?.split('T')[0]}</td>
                    <td className="px-4 py-3 text-sm text-gray-500">{email.thread_id}</td>
                    <td className="px-4 py-3 text-sm text-gray-500 max-w-md truncate">{email.body?.substring(0, 100)}...</td>
                  </tr>
                ))}
              </tbody>
            </table>
